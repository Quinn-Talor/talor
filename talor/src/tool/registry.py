...
following opencode's ToolRegistry pattern.
...
from talor.tool.tool import ToolInfo
...
class ToolRegistry:
    ...
    Corresponds to opencode's ToolRegistry namespace.
    ...
    async def register(
        ...
        Corresponds to opencode's tool auto-registration.
        ...
        async with self._lock:
            ...
            # Publish event
            if self._bus:
                from talor.bus.events import ToolRegistered, ToolRegisteredData
    ...
    async def unregister(self, tool_name: str) -> None:
        ...
        async with self._lock:
            ...
            # Publish event
            if self._bus:
                from talor.bus.events import ToolUnregistered, ToolUnregisteredData
                await self._bus.publish(
                    ToolUnregistered,
    ...
    async def list(
        ...
        Corresponds to opencode's tool list.
    ...
    async def get_llm_definitions(
        ...
        Corresponds to opencode's prepareResponsesTools().
    ...
    async def execute(
        ...
        if self._bus:
            from talor.bus.events import ToolExecuting, ToolExecutingData
            await self._bus.publish(
                ToolExecuting,
                ToolExecutingData(
        ...
        try:
            ...
            if self._bus:
                from talor.bus.events import ToolExecuted, ToolExecutedData
                await self._bus.publish(
                    ToolExecuted,
                    ToolExecutedData(
                        session_id=context.session_id,
