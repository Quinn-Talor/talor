...
from src.session.session import Session, SessionInfo, SessionBusyError
...
if TYPE_CHECKING:
    ...
    from src.tool import ToolRegistry, ToolContext
...
class SessionPrompt:
    ...
    Provides the main event loop for processing prompts in the ReAct cycle.
    """
    ...
    async def _process_step_stream(
        ...
        try:
            ...
            if cls._bus:
                from src.bus.events import AgentError, AgentErrorData, StreamError, StreamErrorData
                await cls._bus.publish(
                    AgentError,
                    AgentErrorData(
                        session_id=session_id,
