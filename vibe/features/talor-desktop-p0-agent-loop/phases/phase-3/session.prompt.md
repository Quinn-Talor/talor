# Phase 3 会话执行 Prompt
# talor-desktop P0 Agent Loop — 高风险工具确认流程 + UI

> **前置要求**：Phase 2 certificate 已签收，ContentBlock 写库已工作，ReAct loop 已实时入库。
...
// ✅ 正确 - 禁用按钮
<button disabled={streamState === 'streaming'} onClick={handleSend}>发送</button>
```
...
1. 你必须在 `electron.vite.config.ts` 为 main 和 renderer 都添加 `@shared` 别名
2. 你必须注意 Vercel AI SDK `role: 'tool'` 消息格式：content 必须是 `{ type: 'tool-result', toolCallId, toolName, result }` 数组
3. 你在清库迁移后保留 try/catch 降级到 `[{type:'text', text: content}]` 作为防御
4. 你必须在 `await result.toolResults` 之后才能写库
...
| 超时测试 | 触发确认框后等待 30 秒观察自动拒绝 |

---

## §七 会话状态

**当前 Phase**：Phase 3 — 高风险工具确认流程 + UI

**任务队列**：
1. ⬜ IMPL-006：工具分级 + confirm IPC 流程（主进程侧）
2. ⬜ IMPL-007：ToolConfirmDialog UI + chatStore 状态

**阶段退出标准**：用户触发 bash/write/edit → 看到 ToolConfirmDialog → 点击"执行"或"拒绝"→ 工具按预期执行或跳过（AC-003-01 ~ AC-003-06 通过）

**本 Phase 完成后**：执行迭代归档协议，全部 P0 特性上线。
