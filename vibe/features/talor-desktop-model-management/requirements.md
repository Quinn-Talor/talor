...
| 用户在现有会话中切换模型 | 系统直接切换模型，不弹窗确认，不清空会话历史，显示"已切换模型"提示 |
...
- **Then**：系统直接切换，不弹出任何确认对话框
- **Then**：会话 model_id 更新为"openai/gpt-4o"
- **Then**：会话历史不清空，原有消息保留
- **Then**：页面显示"已切换模型"提示
- **关联 US**：US-012
...
- **Then**：系统直接切换，不弹出任何确认对话框
- **Then**：图片附件静默忽略（不影响切换流程）
- **Then**：切换成功后显示"已切换模型"提示
- **关联 US**：US-012
