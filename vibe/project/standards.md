...
### A-MUST-3 · 注释只描述当前代码现状，不留历史变更或旧备注

代码注释禁止包含以下信息（即便临时撰写）:

- **历史改造痕迹**: "v3 → v4", "Phase 1B 优化后", "(v3 沿用)", "v3.7 移除", "(2026-05-14 改造)"
- **commit 引用**: "见 commit 77cd8bd", "Phase 3 删除"
- **过时"曾经"叙述**: "原本 X, 现在 Y", "之前 A, 改为 B"
- **未来 TODO 计划**: "Phase 2b 会删", "v5 重构"
- **修复痕迹**: "(踩坑总结: ...)", "(本次 bug fix)"

应只写: **代码为何这样写**（WHY 非显然时） + **关键约束 / 不变量 / 边界**。

- 依据：`src/main/loop/react-loop.ts:1-22`（v4.2 重写后的文件头注释 — 描述设计取舍但**未来要清理为纯当前状态描述**）
- 违反后果：注释随版本演进迅速过时；新读者被旧叙述误导；commit message + git blame + plans/ 才是历史档案的正当承载
- 历史信息归属:
  - **commit message** = 单次改动 why
  - **git log / git blame** = 改动溯源
  - **docs/superpowers/plans/** = 大型改造的"为什么这么改"的长期档案
  - **vibe/project/** = L1 长期规范
  - **代码注释** = 仅当前代码的"为什么这样写"
- → `patterns.md` §P7
...
### F-MUST-5 · 工具失败必须把命令实际输出原样带回(Principle 3)

bash 等工具非零退出时返回 **stdout + stderr** 合并,不能只取 stderr。

- 依据:`src/main/tools/builtin/bash.ts` close handler(`code !== 0` 分支合并 stdout+stderr)
- 背景:CLI 把错误打 stdout、或模型用 `2>&1` 合并流时,只取 stderr 会让模型只看到 `exit code N` 无从诊断 → 盲目重试 → failure-streak 收尾,任务卡死
- 守护:`bash.test.ts`(stdout 诊断 / `2>&1` 合并流)
...
### F-NEVER-3 · 禁止使用 `streamObject` (v4)

SDK v7 已 `@deprecated`("Use streamText with an output setting instead")。
Talor 永不引入。如需结构化输出:

- 非流式 / 短小:`generateObject` (v4 Phase 5 用于 memory 压缩)
- 流式 + tools / 多步:`streamText({ experimental_output: Output.object({ schema }) })`
...
依据:`docs/superpowers/plans/2026-05-14-talor-v4-sdk-native.md` §3.4
...
### I-MUST-3 · 读取 messages 必须 `ORDER BY created_at ASC, rowid ASC`

- 依据：`src/main/repos/session-repo.ts:listBySession`
- 背景：`createBatch` 给同批 `assistant(tool_use)` + `tool(result)` 盖**同一** `created_at`。只按 created_at 排序时二者顺序由 SQLite 任意决定,可能反转 → tool_result 排到 tool_use 前 → SDK `AI_MissingToolResultsError`(I-MUST-1 的孪生)。rowid 是单调插入序 = 正确配对序。
- 违反后果：工具回合随机崩；v7 严格校验下确定性崩
- 守护：`session-repo-message-order.test.ts`
...
### J-MUST-2b · prompt 重建不得在 `tool_use` 与 `tool_result` 之间插入消息

append-only 分层装配里,history 末尾的 `assistant(tool_use)`(MemoryPlugin)与当前 turn 的 `tool(result)`(MessagePlugin)必须**相邻**。volatile 层的「线程外旁注」(`RuntimeMetaPlugin` 的 date/os/workspace、reflector hint、`[DEGRADED]`)必须排在 current-turn **之后**,不得劈开配对。

- 依据:`src/main/prompt/PromptPipeline.ts:getPlugins`(MessagePlugin 排在 RuntimeMetaPlugin 前)
- 违反后果:SDK `convertToLanguageModelPrompt` 抛 `AI_MissingToolResultsError`,每个工具回合续做步必崩(Phase 1 重排曾踩,v7 暴露)
- 守护:`PromptPipeline.test.ts`(tool_use/tool_result 相邻)、`prompt-cache-stability.test.ts`(稳定前缀字节一致)

### J-MUST-2c · 缓存前缀(稳定层)跨 build 必须字节一致

进入可缓存前缀的 system/agent/tools/history 层不得含每轮变化的内容(毫秒时间戳、随机数、自增计数)。易变内容归 volatile 尾部(`RuntimeMetaPlugin`)。

- 依据:`src/main/prompt/cache-breakpoints.ts`、`buildRuntimeContext`(纯静态)
- 背景:历史上 SystemPlugin 的 `Current time` 毫秒戳把 deepseek 前缀缓存命中率从 ~99% 打到 ~14%
- 守护:`prompt-cache-stability.test.ts`、`runtime-context.test.ts`(determinism)
...
| 工具失败重试                       | ✅(自适应)                                      | ✅(死循环兜底)                                     |
| **声明副作用 (v4)**                | —(SDK tool call 即声明)                         | ✅ tool({ needsApproval }) + regex fallback        |
| 副作用授权决策                     | —                                               | ✅(SDK approval 流程 + memory)                     |
| **请求续做 (v4)**                  | ✅ 调 request_continuation tool                 | ✅ SDK 多步自动续                                  |
| **该不该终止 turn (一审)**         | ✅ LLM 自陈 (block + SDK finishReason)          | ✅ 系统消费 SDK 结构信号                           |
| **该不该终止 turn (二审 v3.7.3+)** | ✅ LLM-as-judge                                 | ✅ 调度二审 + 注入续做提示                         |
| 任务完成判断                       | ✅                                              | —                                                  |
| 需要用户输入                       | ✅(自然语言)                                    | —                                                  |
| 任务被卡住                         | ✅(自然语言)                                    | —                                                  |
| UI 卡片样式                        | 可选(主动 emit done/need_input/blocked/warning) | ✅(推断兜底,**仅 UI 不回馈 loop**)                 |
| 事实引用准确性                     | 推理                                            | ✅(verify-quote 核对)                              |
| **Token 预算 (v4)**                | —                                               | ✅ SDK `usage.inputTokens` 精确值(替代 estimate)   |
| **Provider 警告 (v4)**             | —                                               | ✅ SDK `warnings` 入 log(v4.1 入 Ledger)           |
| **Cache 命中观测 (v4)**            | —                                               | ✅ SDK `providerMetadata` (cacheReadInputTokens)   |
| Memory 压缩                        | —                                               | ✅ generateObject + CompressionSchema (v4 Phase 5) |
| 跨 turn 状态                       | —                                               | ✅(持久化)                                         |
...
- v4 SDK-native 改造: [docs/superpowers/plans/2026-05-14-talor-v4-sdk-native.md](../../docs/superpowers/plans/2026-05-14-talor-v4-sdk-native.md)
- → `patterns.md` §P13

### J-SHOULD-3 · AI SDK 信号按"LLM 自陈 / 运行时真相"两类消费 (v4)

AI SDK 给我们的所有 metadata 信号按 J-SHOULD-2 分类纳入消费:
...
- text 内 talor block (UI 装饰类:done/need_input/blocked/warning)
  → 处理方式: policy 链直接消费,信任 LLM,系统不二次推断

**类别 B — 运行时真相** (provider/SDK 测得):

- `usage`: { inputTokens, outputTokens, totalTokens, **inputTokenDetails: { noCacheTokens, cacheReadTokens, cacheWriteTokens }** }(v7 统一缓存字段;`usage-normalizer` 优先读它,回退扫 providerMetadata)
- `durationMs`
- `providerMetadata`: { anthropic.cacheReadInputTokens, deepseek.promptCacheHitTokens, ... }(归一回退源)
- `warnings`: provider 警告
  → 处理方式: detector / Ledger / `usage-recorder`→session 表 / observability 消费,系统兜底/审计
