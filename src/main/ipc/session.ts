...
// 允许依赖：providers/*、services/*、repos/*、shared/*    禁止依赖：loop/*

import { ipcMain } from 'electron'
import { sessionRepo, messageRepo, ChatSession, ChatMessage } from '../repos/session-repo'
import { ConfigStore } from '../store/config-store'
import { getProviderModels } from '../providers/provider-fetcher'
import { checkModelAvailability } from '../providers/model-availability'
...
export function registerSessionHandlers(): void {
  ...
  ipcMain.handle(
    'session:create',
    async (_, params: { provider_id: string; model_id?: string }): Promise<ChatSession> => {
      let modelId = params.model_id

      if (!modelId) {
        const providers = ConfigStore.getInstance().get('providers') as Record<
          string,
          { id: string; base_url: string; type: string }
        >
        const provider = providers?.[params.provider_id]
        if (provider) {
          const models = await getProviderModels(
            provider as Parameters<typeof getProviderModels>[0],
          )
  ...
  ipcMain.handle(
    'session:rename',
    (_, params: { session_id: string; title: string }): ChatSession | null => {
  ...
  ipcMain.handle(
    ...
    (
      _,
      params: { session_id: string; model_id: string; provider_id?: string },
    ): ChatSession | null => {
      if (params.provider_id) {
        sessionRepo.updateProvider(params.session_id, params.provider_id)
      }
  ...
  ipcMain.handle(
    ...
    (_, params: { session_id: string; workspace: string }): ChatSession | null => {
      return sessionRepo.updateWorkspace(params.session_id, params.workspace)
    },
  )

  ipcMain.handle(
    'session:checkModelAvailability',
    async (
      _,
      ...
      const result = checkModelAvailability(
        session.model_id,
        models as Parameters<typeof checkModelAvailability>[1],
      )
      log.info(
        '[session:checkModelAvailability] session:',
        params.session_id,
        'model:',
        session.model_id,
        'available:',
