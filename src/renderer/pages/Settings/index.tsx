    ...
    <div className="flex flex-col h-full" style={{ background: '#f4f6f8' }}>
      ...
      <div className="shrink-0 bg-white" style={{ borderBottom: '1px solid #e8eaed' }}>
        <div
          ...
          style={{ height: 52, display: 'flex', alignItems: 'center', gap: 12 }}
        >
          <button
            onClick={onBack}
      ...
      <div className="shrink-0 bg-white" style={{ borderBottom: '1px solid #e8eaed' }}>
        <div className="max-w-4xl mx-auto px-8 flex items-center">
          ...
          {activeTab === 'provider' && formMode === 'closed' && <AddBtn onClick={openCreateForm} />}
          ...
          {activeTab === 'account' && <AddBtn onClick={() => setAccountAddTrigger((t) => t + 1)} />}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-8">
        <div className="max-w-4xl mx-auto px-8">
            ...
            <div className="space-y-4">
              ...
              {formMode === 'closed' &&
                (providers.length === 0 ? (
                  <EmptyState
                    message="暂无 Provider"
                    action={{ label: '新增 Provider', onClick: openCreateForm }}
                  />
                ) : (
                  <ProviderList
                    providers={providers}
          ...
          {/* Account */}
          {activeTab === 'account' && <AccountsSettings addTrigger={accountAddTrigger} />}
...
function SettingRow({ label, value, last }: { label: string; value: string; last?: boolean }) {
  return (
    <div
      className={`flex items-center justify-between px-5 py-3.5 ${!last ? 'border-b border-gray-100' : ''}`}
    >
