...
import { applyManualCapabilities } from './capability-updater'
import type { ModelCapability } from '../types/models'

const BASE_CAPS: ModelCapability[] = [
  { category: 'text', type: 'text_generation', supported: true, description: '文本生成', source: 'auto' },
  { category: 'vision', type: 'image_understanding', supported: false, description: '图片理解', source: 'auto' },
]
...
describe('applyManualCapabilities', () => {
  ...
  it('sets detected_at timestamp on all capabilities', () => {
    const result = applyManualCapabilities(BASE_CAPS)
    expect(result.every(c => typeof c.detected_at === 'string' && c.detected_at.length > 0)).toBe(true)
  })

  it('preserves supported flag from input', () => {
    const caps: ModelCapability[] = [
      { category: 'vision', type: 'image_understanding', supported: true, description: '图片理解', source: 'auto' },
    ]
  ...
  it('overrides existing source=default to manual', () => {
    const caps: ModelCapability[] = [
      { category: 'text', type: 'text_generation', supported: true, description: '文本', source: 'default' },
    ]
    const result = applyManualCapabilities(caps)
